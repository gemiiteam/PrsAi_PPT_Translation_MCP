...
- `src/my_python_mcp/server.py`：定义了 MCP 服务器实例，并使用 `@mcp.tool()` 注册了示例工具。**你的核心业务逻辑写在这里**。
- `src/my_python_mcp/__main__.py`：程序的入口点。
...
```bash
uv run my-python-mcp
# 或者
python -m my_python_mcp
```
